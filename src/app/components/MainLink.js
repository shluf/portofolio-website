    ...
    >
      ...
      <div className="sm:flex sm:flex-row justify-between items-start mb-10">
        <p className={`mx-3 sm:flex sm:items-center hidden`}>
          {/* &gt;&nbsp; */}
          <motion.span
            variants={gridMenuContent}
            animate={onHover === true ? "showLeft" : "hideLeft"}
        ...
        <motion.p
          variants={gridMenuContent}
          animate={onHover === true ? "showRight" : "hideRight"}
          transition="transition"
          className={`${
            router === href ? "bg-light text-dark" : "border-solid border-light"
          } rounded-b-xl p-4 mr-3 sm:flex sm:items-center hidden text-3xl`}
        >
          {num}
        </motion.p>
      </div>
      <p
        className={`${className} group-hover:font-extrabold mb-4 relative text-center transition-all ease duration-300 ${
          router === href ? "font-medium" : "font-normal"
        }`}
      >
        {title}
        <span
...
const MainLink = () => {
  ...
  const buttonNav =
    "flex items-center justify-center hover:bg-light hover:text-dark hover:font-bold py-3 px-5 transition-all-ease duration-100";
  ...
  return (
    <nav className="w-full sm:flex sm:flex-col sm:items-end sm:justify-end sm:bottom-[unset] sm:py-3 py-0 fixed bottom-0 z-[50]">
      <div
        ...
        className={`${
          menuCheck ? "fixed" : "hidden"
        } left-0 top-0 h-screen w-screen bg-[#000000a9] -z-[20]`}
      />
      <div className="hidden sm:flex items-center rounded-xl sm:mx-5 text-dark ">
        <motion.button
          whileHover={{ x: -10 }}
          className={`${
            menuCheck ? "text-light" : "text-dark"
          } flex items-center justify-end py-1 px-5 rounded-l-xl group duration-50`}
        >
          Let&apos;s Connect
          <span className="group-hover:opacity-100 group-hover:ml-2 ml-0 opacity-0">
            &#62;
          </span>
        </motion.button>
        <div className="hidden sm:flex items-center rounded-md  bg-dark text-light ">
          <button className={`${buttonNav} rounded-l-md`}>Dark</button>
          <button
